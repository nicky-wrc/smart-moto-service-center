...
export class PurchaseReceivesService {
  ...
  async create(
    ...
    if (!po)
      throw new NotFoundException(
        `PurchaseOrder ${data.purchaseOrderId} not found`,
      );
    if (po.status !== 'ORDERED' && po.status !== 'PARTIAL_RECEIVED') {
      throw new BadRequestException(
        'PO must be in ORDERED or PARTIAL_RECEIVED status to receive goods',
      );
    }
    ...
    for (const receiveItem of data.items) {
      ...
      if (!poItem) {
        throw new BadRequestException(
          `PO Item ID ${receiveItem.purchaseOrderItemId} not found in this PO`,
        );
      }
        ...
        data: {
          ...
          items: {
            create: data.items.map((i) => {
              const poItem = po.items.find(
                (pi) => pi.id === i.purchaseOrderItemId,
              )!;
              return {
  ...
  async findOne(id: number) {
    ...
    if (!receive)
      throw new NotFoundException(`PurchaseReceive ${id} not found`);
    return receive;
  }
}
