...
export class ReportsController {
    ...
    @ApiQuery({ name: 'date', required: false, description: 'YYYY-MM-DD (default: today)' })
    ...
    getProfitReport(
        ...
        return this.reportsService.getProfitReport(new Date(dateFrom), new Date(dateTo));
    }
    ...
    getBestSellersReport(
        ...
        return this.reportsService.getBestSellersReport(new Date(dateFrom), new Date(dateTo));
    }

    @Get('stock-non-moving')
    @Roles('MANAGER', 'ADMIN', 'STOCK_KEEPER')
    @ApiOperation({ summary: 'อะไหล่ไม่เคลื่อนไหว (ไม่มี movement ในช่วงเวลาที่กำหนด)' })
    @ApiQuery({ name: 'days', required: false, description: 'จำนวนวันย้อนหลัง (default: 90)', example: 90 })
