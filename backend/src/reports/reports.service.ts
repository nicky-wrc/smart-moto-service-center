...
export class ReportsService {
    ...
    async getDailyReport(date: Date) {
        ...
        const [payments, newJobs, completedJobs, cancelledJobs] = await Promise.all([
            this.prisma.payment.findMany({
                ...
                include: {
                    job: { include: { laborTimes: true, partRequisitions: { include: { items: { include: { part: true } } } } } },
                },
            }),
            ...
            this.prisma.job.count({
                where: { completedAt: { gte: startOfDay, lte: endOfDay }, status: { in: [JobStatus.COMPLETED, JobStatus.PAID] } },
            }),
            this.prisma.job.count({
                where: { updatedAt: { gte: startOfDay, lte: endOfDay }, status: JobStatus.CANCELLED },
            }),
        ]);
    ...
    async getMonthlyReport(month: number, year: number) {
        ...
        const [payments, newJobs, completedJobs, cancelledJobs] = await Promise.all([
            ...
            this.prisma.job.count({
                where: { completedAt: { gte: startOfMonth, lte: endOfMonth }, status: { in: [JobStatus.COMPLETED, JobStatus.PAID] } },
            }),
            this.prisma.job.count({
                where: { updatedAt: { gte: startOfMonth, lte: endOfMonth }, status: JobStatus.CANCELLED },
            }),
        ]);
        ...
        payments.forEach((p) => {
            if (p.paidAt) {
                ...
                dailyBreakdown[day] = (dailyBreakdown[day] || 0) + Number(p.totalAmount);
            }
        });
            ...
            if (payment.job) {
                for (const req of payment.job.partRequisitions) {
                    for (const item of req.items) {
                        if (item.part) {
                            totalPartsCost += Number(item.part.unitPrice) * item.issuedQuantity;
                        }
                    }
                }
    ...
    async getPendingJobs() {
        ...
        const counts = await Promise.all(
            statuses.map((status) =>
                this.prisma.job.count({ where: { status } }),
            ),
        );
            ...
            if (emp.role === 'TECHNICIAN' || emp.role === 'FOREMAN') {
                ...
                if (emp.commissionRate) {
                    for (const job of completedJobs) {
                        if (job.payment && job.payment.paymentStatus === PaymentStatus.PAID) {
                            commission += Number(job.payment.totalAmount) * Number(emp.commissionRate) / 100;
                        }
                    }
                }
            }
    ...
    async getBestSellersReport(dateFrom: Date, dateTo: Date) {
        ...
        const partMap: Record<number, { partNo: string; name: string; category: string | null; totalOut: number }> = {};
