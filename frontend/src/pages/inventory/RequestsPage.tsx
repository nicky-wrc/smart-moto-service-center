          ...
          <div className="relative mt-2 p-5 bg-white rounded-xl border border-gray-200 shadow-md">
            <div className="relative z-30 grid grid-cols-1 md:grid-cols-4 gap-6">
              <div>
                ...
                <select value={filterRequester} onChange={(e) => setFilterRequester(e.target.value)} className="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500">
              ...
              <div>
                ...
                <select value={filterModel} onChange={(e) => setFilterModel(e.target.value)} className="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500">
                  <option value="">ทั้งหมด</option>
              ...
              <div>
                ...
                <input type="time" value={filterTime} onChange={(e) => setFilterTime(e.target.value)} className="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500 mb-1">ป้ายทะเบียน (บรรทัดบน)</label>
                <input type="text" placeholder="เช่น 1กข" value={filterPlateLine1} onChange={(e) => setFilterPlateLine1(e.target.value)} className="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500 mb-1">ป้ายทะเบียน (บรรทัดล่าง)</label>
                <input type="text" placeholder="เช่น 1234" value={filterPlateLine2} onChange={(e) => setFilterPlateLine2(e.target.value)} className="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500 mb-1">จังหวัด</label>
                <select value={filterProvince} onChange={(e) => setFilterProvince(e.target.value)} className="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500">
                  <option value="">ทั้งหมด</option>
              ...
              >
                <div className="flex items-stretch">
                  ...
                  <div className="flex-1 flex justify-end items-center pr-4">
                    <span className="text-sm text-gray-400">{req.requestedAt}</span>
                  </div>
                </div>
                <div className="bg-white px-5 py-4 grid grid-cols-1 gap-y-2">
                  <div className="flex items-center gap-2 text-sm text-gray-700">
                    ...
                    <span><span className="text-gray-400">ผู้ทำเรื่องเบิก : </span>{req.requester} ({req.requesterRole})</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-gray-700">
                    ...
                    <span><span className="text-gray-400">รุ่นรถ : </span>{req.motorcycleModel}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-gray-700">
                    ...
                    <span><span className="text-gray-400">ป้ายทะเบียน : </span>{req.licensePlate}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-gray-700">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-amber-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                    </svg>
                    <span><span className="text-gray-400">รายการคำขอ : </span>{req.items.length} รายการ ({req.items.reduce((s, i) => s + i.quantity, 0)} ชิ้น)</span>
                  </div>
