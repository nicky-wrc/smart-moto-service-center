import { useLocation, Outlet } from 'react-router-dom'
import AppHeader from '../../components/AppHeader'
import Sidebar from '../../components/Sidebar'
import type { NavItem } from '../../components/Sidebar'

const navItems: NavItem[] = [
  {
    path: '/foreman/dashboard',
    label: 'ภาพรวม',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M4 5a1 1 0 011-1h4a1 1 0 011 1v5a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h4a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM14 5a1 1 0 011-1h4a1 1 0 011 1v3a1 1 0 01-1 1h-4a1 1 0 01-1-1V5zM14 12a1 1 0 011-1h4a1 1 0 011 1v8a1 1 0 01-1 1h-4a1 1 0 01-1-1v-8z" />
      </svg>
    ),
  },
  {
    path: '/foreman/jobs',
    label: 'รายการใบงานซ่อม',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
      </svg>
    ),
  },
  {
    path: '/foreman/mechanics',
    label: 'ช่างและงานที่รับผิดชอบ',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
      </svg>
    ),
  },
  {
    path: '/foreman/history',
    label: 'ประวัติใบงาน',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
    ),
  },
]

const pageTitles: Record<string, string> = {
  '/foreman/dashboard': 'ภาพรวม',
  '/foreman/jobs': 'รายการใบงานซ่อมทั้งหมด',
  '/foreman/mechanics': 'ช่างและงานที่รับผิดชอบ',
  '/foreman/history': 'ประวัติใบงาน',
}

export default function ForemanLayout() {
  const location = useLocation()
  const title = Object.entries(pageTitles).find(([path]) => location.pathname.startsWith(path))?.[1] ?? 'หัวหน้าช่าง'

  return (
    <div className="h-screen overflow-hidden bg-[#44403C] pb-6 pr-6 flex items-stretch font-[Kanit]">
      <div className="flex-1 bg-[#44403C] rounded-2xl flex flex-col overflow-hidden">
        <AppHeader title={title} />
        <div className="flex flex-1 gap-0 overflow-hidden">
          <Sidebar navItems={navItems} />
          <div className="flex-1 bg-[#F5F5F5] rounded-xl overflow-hidden">
            <Outlet />
          </div>
        </div>
      </div>
    </div>
  )
}
